...
function renderPage(sort_array,$data_set,$size,$time,grid_width){
    ...
    bubbleSort(random_numbers,sort_array);
...
//sorting algorithm
function bubbleSort(array,sort_array){
    for(var iteration = 0 ; iteration < array.length - 1; iteration++){
        for(var j = 0 ; j < array.length - iteration - 1; j ++){
            if(array[j] > array[j+1]){
                swap(array,sort_array,j,j+1);
            }else{
                sort_array.push(["compare",j,j+1]);
            }
        };
    };
}